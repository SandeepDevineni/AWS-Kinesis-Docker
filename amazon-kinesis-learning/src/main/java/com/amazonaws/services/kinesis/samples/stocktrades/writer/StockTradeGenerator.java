...
/**
 * Generates random stock trades by picking randomly from a collection of stocks, assigning a random
 * price based on the mean, and picking a random quantity for the shares.
 */
  ...
  public StockTrade getRandomTrade() {
    ...
    long quantity =
        random.nextInt(MAX_QUANTITY)
            + 1; // add 1 because nextInt() will return between 0 (inclusive)
    // and MAX_QUANTITY (exclusive). we want at least 1 share.

    return new StockTrade(
        stockPrice.tickerSymbol, tradeType, price, quantity, id.getAndIncrement());
  }
